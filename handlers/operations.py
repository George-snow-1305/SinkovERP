...
from schemas.operations import (FolderItem,
...
TABLE_OPERATIONS = "catalog_operations"
TABLE_OPERATIONS_FOLDERS = "catalog_operations_folders"
TABLE_OPERATIONS_FOLDERS_STRUCTURE = "catalog_operations_folders_structure"


@router.get('/get_folders', response_model=FolderInStructureResponseBody)
async def get_folders_stucture(folder_id: int):
    connection = DatabaseConnector()
    query = GET_FOLDER_BY_FOLDER_ID.format(table_folders=TABLE_OPERATIONS_FOLDERS, folder_id=folder_id)
    print(query)
    ...
    if len(result) == 0:
        ...
        connection = DatabaseConnector()
        query_child = GET_CHILD_FOLDERS.format(table_folders=TABLE_OPERATIONS_FOLDERS,
                                               table_folders_structure=TABLE_OPERATIONS_FOLDERS_STRUCTURE,
                                               folder_id=folder_id)
        print(query_child)
        ...
        query_parents = GET_PARENT_FOLDERS.format(table_folders=TABLE_OPERATIONS_FOLDERS,
                                                  table_folders_structure=TABLE_OPERATIONS_FOLDERS_STRUCTURE,
                                                  folder_id=folder_id)
...
async def create_folder(body: CreateFolderRequestBody):
    ...
    query = GET_FOLDER_BY_FOLDER_ID.format(table_folders=TABLE_OPERATIONS_FOLDERS, folder_id=body.parent,)
    ...
    query = CREATE_FOLDER.format(name=body.name,
                                 color=body.color,
                                 parent=body.parent,
                                 table_folders=TABLE_OPERATIONS_FOLDERS,
                                 table_folders_structure=TABLE_OPERATIONS_FOLDERS_STRUCTURE
                                 )
